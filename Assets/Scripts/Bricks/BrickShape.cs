...
public class BrickShape : MonoBehaviour
{
    ...
    public void SetAssetGameobject (GameObject mesh, Texture2D texture) {
        if (assetGO != null)
        {
            Destroy(assetGO);
        }

        assetGO = mesh.transform.GetChild(0).gameObject;
        assetGO.name = "Asset";
        ...
        assetGO.transform.localRotation = Quaternion.Euler(0, 180, 0);
        Destroy(mesh);
        ...
        if (brick.Material != null && brick.Material != "")
        {
            GameObject manager = GameObject.Find("Manager");
            if (manager != null)
            {
                MapBuilder mb = manager.GetComponent<MapBuilder>();
                if (mb.materials.ContainsKey(brick.Material))
                {
                    Material mat = mb.materials[brick.Material];
                    Material[] mats = new Material[] { mat, mat, mat };
                    if (brick.BrickColor.a != 1) for (int i = 0; i < mats.Length; i++) mats[i].shader = mb.AlphaShader; // transparent geometry uses a different shader
                    mr.materials = mats;
                }
            }
        }
    ...
    public void SetAssetGameobjectOld (Mesh mesh, Texture2D texture) {
        if (assetGO == null) {
